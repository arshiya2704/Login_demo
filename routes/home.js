...
function afterSignIn(req,res)
{
	...
	var getUser="select * from userenteries where uname='"+req.param("inputUsername")+"' and password='" + req.param("inputPassword") +"'";
...
{
	var getAllUsers = "select * from userenteries";
...
{
	// check user already exists
	var getUser="insert into userenteries values( '"+req.param("FirstName") +"' ,'"+ req.param("LastName") +"' ,'" + req.param("inputUsername")+"' ,'"+ req.param("inputPassword")+"' ,'"+ req.param("DOB")+"' ,'"+ req.param("Gender")+"' );";
	console.log("Query is:"+getUser);
			...
			throw err;
				...
				ejs.renderFile('./views/register.ejs',function(err, result) {
			        ...
			        if (!err) {
			        		...
			        		var popUp = 	"<script>$(\"#myModal\").modal();</script>";
			            res.end(result + popUp);
			        }
			        // render or error
			        else {
			            res.end('An error occurred');
